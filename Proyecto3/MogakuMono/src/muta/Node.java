...
public class Node {
	...
	// Increasing the depth of the tree seemed to have some positive
	// improvements.
	 ...
	 *
	 * @param fd
	 *            frame data (including information such as hp)
	 * @return the evaluation value
	 */
	...
	public int getScore(FrameData fd) {
		...
		int score = playerNumber
				? offenseC * (fd.getCharacter(true).getHp() - myOriginalHp)
						- defenseC * (fd.getCharacter(false).getHp() - oppOriginalHp)
				: offenseC * (fd.getCharacter(false).getHp() - myOriginalHp)
						- defenseC * (fd.getCharacter(true).getHp() - oppOriginalHp);
		// Constant values to make crouch guard and stand guard used more
		// frequently.
		// Ideally I would use an algorithm for this and not hard code it, but
		// it works
		// extremely well at the moment, and time constraints leave me with no
		// option for
		// implementing an algorithm.
		if (fd.getCharacter(playerNumber).getAction().name() == "CROUCH_GUARD"
				|| fd.getCharacter(playerNumber).getAction().name() == "STAND_GUARD") {
			score = 10;
		}
		return score;
	}
	...
	/**
	 * Return the UCB1 value calculated from the evaluation value, the total
	 * number of playouts(trails), and the number of playouts of the
	 ...
	 * @param ni
	 *            The number of playouts of the corresponding action
	 * @return UCB1 value
	 */
