...
public class Node {
	...
	public Node(long ucttime, FrameData frameData, Node parent, LinkedList<Action> myActions,
			LinkedList<Action> oppActions, GameData gameData, boolean playerNumber, CommandCenter commandCenter,
			LinkedList<Action> selectedMyActions) {
		this(ucttime, frameData, parent, myActions, oppActions, gameData, playerNumber, commandCenter);
	...
	public double playout() {
		...
		for (int i = 0; i < 5; i++) {
			Action toac;
			toac = oppActions.get(rnd.nextInt(oppActions.size()));
	...
	public void createNode() {
		...
		for (int i = 0; i < children.length; i++) {
			...
			children[i] = new Node(this.UCT_TIME, frameData, this, myActions, oppActions, gameData, playerNumber,
					commandCenter, my);
		}
	}

	public void createNode(LinkedList<Action> myacs) {

		this.children = new Node[myacs.size()];
	...
	public Action getBestVisitAction() {
		...
		return children[selected].selectedMyActions.getFirst();
	}
